...
def baked_goods():
    ...
    if request.method == 'GET':
        ...
        resp = make_response(new_good_dict, 201)
        return resp
    
@app.route('/baked_goods/<int:id>', methods = ['GET', 'DELETE'])
def baked_good_by_id(id):
    bgood = BakedGood.query.filter_by(id=id).first()

    if request.method == 'GET':
        bgood_dict = bgood.to_dict()

        resp = make_response(bgood_dict, 200)
        return resp
    
    elif request.method == 'DELETE':
        db.session.delete(bgood)
        db.session.commit()
